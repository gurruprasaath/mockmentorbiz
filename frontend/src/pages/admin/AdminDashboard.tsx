...
import {
  ...
  AlertCircle, Zap, ChevronRight, Star, CalendarPlus,
  UserCheck,
} from 'lucide-react'
...
const initials = (name?: string) =>
  name ? name.split(' ').map((n: string) => n[0]).join('').toUpperCase().slice(0, 2) : 'AD'
...
export default function AdminDashboard() {
  ...
  const students    = studentsData?.data?.slice(0, 5) || []
  const scheduled   = scheduledData?.data?.slice(0, 4) || []
  const malpractice = malpracticeData?.data || []

  /* derived stats */
  const totalStudents    = d.total_students || 0
  const recentInterviews = d.recent_interviews || 0
  const avgPerf = parseFloat((
    ((d.average_performance?.technical || 0) + (d.average_performance?.communication || 0)) / 2
  ).toFixed(1))
  const malpracticeCount = d.malpractice_incidents || 0

  /* critical = malpractice with severity high */
  const criticalCount  = malpractice.filter((r: any) => r.severity === 'high').length
  const pendingCount   = malpractice.filter((r: any) => !r.reviewed).length
  const warningCount   = malpractice.filter((r: any) => r.severity === 'medium').length

  /* animated counters */
  const cStudents   = useCounter(totalStudents)
  const cInterviews = useCounter(recentInterviews)
  const cPerf       = useCounter(avgPerf)
  const cMalpractice = useCounter(malpracticeCount)

  const today = new Date().toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })

  /* avg perf score as 0–10 for donut */
  const perfScore = parseFloat(((avgPerf / 100) * 10).toFixed(1))
  ...
  return (
    <div className="min-h-screen" style={{ background: 'linear-gradient(135deg, #f0f4ff 0%, #fafafa 50%, #f5f0ff 100%)' }}>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="hidden sm:block">
              ...
              <p className="text-[11px] text-gray-400 mt-0.5">{user?.department ? `${user.department} Admin` : 'Department Admin'}</p>
            </div>
          </div>
          <button
            onClick={logout}
      ...
      <div className="max-w-[1320px] mx-auto px-4 sm:px-6 lg:px-10 py-8">
        ...
        {/* ══ ROW 2: Perf tile (dark) + Score + Alerts ══ */}
        <div className="grid grid-cols-12 gap-4 mb-4">

          {/* Avg Performance — dark hero tile */}
          <div
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <div>
                <p className="text-[11px] font-semibold uppercase tracking-widest text-blue-300 mb-1">Performance Overview</p>
                <h3 className="text-white text-[15px] font-semibold">Student Avg Performance</h3>
              </div>
            ...
            <div className="flex items-end justify-between gap-4">
              <div>
                <p className="text-[52px] font-black text-white leading-none tabular-nums tracking-tight">
                  {avgPerf}<span className="text-2xl text-blue-300 font-bold ml-1">%</span>
                </p>
                <p className="text-blue-300 text-xs mt-2">
                  {recentInterviews} interviews in last 30 days
                </p>
              </div>
              <div className="relative flex-shrink-0">
                <Donut pct={avgPerf} color="#3b82f6" size={80} />
            ...
            <div className="mt-5 space-y-2">
              {[
                { label: 'Technical',     val: d.average_performance?.technical || 0,     color: '#3b82f6' },
                { label: 'Communication', val: d.average_performance?.communication || 0, color: '#8b5cf6' },
                { label: 'Confidence',    val: d.average_performance?.confidence || 0,    color: '#10b981' },
              ].map(({ label, val, color }) => (
                <div key={label}>
                  <div className="flex justify-between text-[10px] mb-1">
                    <span style={{ color: '#93c5fd' }}>{label}</span>
                    <span style={{ color: '#93c5fd' }}>{val.toFixed(1)}%</span>
                  </div>
          ...
          >
            <div className="flex items-center justify-between">
              <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Dept Score</p>
              <Star className="h-4 w-4 text-amber-400" />
            </div>
            <div className="flex flex-col items-center justify-center flex-1 py-4">
              <div className="relative">
                <Donut pct={(perfScore / 10) * 100} color="#f59e0b" size={72} />
            ...
            <div className="flex items-center justify-between text-xs">
              ...
              <span className="font-semibold text-gray-700 tabular-nums">{totalStudents}</span>
            </div>
          </div>
          ...
          >
            ...
            <div className="space-y-2 flex-1">
              <AlertTile label="Critical Issues"     value={criticalCount}  severity="critical" />
              <AlertTile label="Pending Reviews"     value={pendingCount}   severity="warning" />
              <AlertTile label="Medium Warnings"     value={warningCount}   severity="info" />
            </div>
            <div className="mt-4 pt-3 border-t border-gray-50">
              <div className="flex items-center gap-1.5 text-xs text-gray-400">
                ...
                <span className="ml-auto font-semibold text-emerald-600">
                  {totalStudents > 0 ? ((malpracticeCount / totalStudents) * 100).toFixed(1) : '0.0'}%
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-12 gap-4 mb-4">
          ...
          <NavTile
            ...
            desc="View and manage your students"
            icon={Users}
            path="/admin/students"
            count={totalStudents}
            countLabel="Students"
          ...
          <NavTile
            ...
            desc="Create and manage interview sessions"
            icon={CalendarPlus}
            path="/admin/schedule"
          ...
          <NavTile
            ...
            desc="Trends, scores and insights"
            icon={BarChart3}
          ...
          <NavTile
            ...
            desc="Review integrity violations"
            icon={Shield}
            path="/admin/malpractice"
            count={malpracticeCount}
            countLabel="Alerts"
            from="#fee2e2" to="#fff1f1"
            iconColor="#dc2626"
          />
        </div>

        {/* ══ ROW 4: Students table + Scheduled interviews ══ */}
        <div className="grid grid-cols-12 gap-4 pb-10">
          ...
          >
            <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom: '1px solid #f8fafc' }}>
              <div>
                <h3 className="text-[14px] font-semibold text-gray-900">Top Students</h3>
              ...
              {studentsLoading ? (
                ...
                <div>
                  ...
                  {students.map((student: any, i: number) => {
                    const score = parseFloat((student.average_score || 0).toFixed(1))
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="col-span-3 text-right text-[13px] font-medium text-gray-700 tabular-nums">
                ...
                <EmptyState icon={Users} label="No students registered yet" cta="Manage students" to="/admin/students" />
              )}
            </div>
          </div>
          ...
          >
            <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom: '1px solid #f8fafc' }}>
              <div>
                <h3 className="text-[14px] font-semibold text-gray-900">Scheduled Interviews</h3>
                <p className="text-xs text-gray-400 mt-0.5">Upcoming and recent sessions</p>
              </div>
              <Link
                to="/admin/schedule"
                className="flex items-center gap-1 text-xs font-semibold text-blue-600 hover:text-blue-700 transition-colors bg-blue-50 hover:bg-blue-100 px-3 py-1.5 rounded-full"
              ...
              {scheduledLoading ? (
                ...
                <div className="space-y-1">
                  {scheduled.map((interview: any, i: number) => {
                    ...
                    const cfg = statusCfg[interview.status] || statusCfg['pending']
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          {initials(interview.student_name)}
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="text-[13px] font-semibold text-gray-800 truncate">{interview.student_name}</p>
                ...
                <EmptyState icon={Calendar} label="No scheduled interviews" cta="Schedule now" to="/admin/schedule" />
              )}
            </div>
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <span className="text-[12px] font-medium text-gray-600">Active Students</span>
              </div>
              <span className="text-[14px] font-black text-blue-600 tabular-nums">{totalStudents}</span>
            </div>
          </div>
        </div>
      </div>
...
function BentoStatCard({ className = '', label, value, suffix, icon: Icon, trend, accentColor, accentLight, sparkColor, up = true }: BentoStatCardProps) {
    ...
    >
      <div className="flex items-start justify-between">
        <div
          ...
          style={{ background: accentLight, transform: hov ? 'scale(1.08)' : 'scale(1)' }}
        >
          <Icon strokeWidth={2} style={{ color: accentColor, width: 18, height: 18 }} />
        </div>
        {trend !== undefined && (
          <span
